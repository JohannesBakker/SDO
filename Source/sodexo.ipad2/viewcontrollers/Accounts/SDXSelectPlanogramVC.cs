...
namespace Sodexo.Ipad2
{
	public partial class SDXSelectPlanogramVC : SDXBaseVC
	{
		...
		{
			SDXAccountDetailVC._selectPlanogramVC = this;
		}
		...
		private UIView CreateDecisionView (DecisionTreeNodeModel node)
		{
			...
			questionLb.Frame = new RectangleF (10, 10, 600, 10);
			...
			view.Frame = new RectangleF (0, 57, 620, answersView.Frame.Y + answersView.Frame.Height + 10);
		...
		private UIView CreateDecisionRadioView (IList<DecisionTreeNodeModel> answerNodes)
		{
			...
			firstSeparatorLine.Frame = new RectangleF (5, 0, 610, 1);
			...
			view.Frame = new RectangleF (0, 0, 620, y);
		...
		private UIView CreateDecisionDropListView (IList<DecisionTreeNodeModel> answerNodes)
		{
			UIView view = new UIView ();
			view.BackgroundColor = UIColor.Clear;
			view.Frame = new RectangleF (0, 0, 620, 30);

			UITextField tf = new UITextField ();
			tf.Frame = new RectangleF (5, 0, 610, 30);
			tf.Text = "";
			...
			iv.Frame = new RectangleF (600, 13, 6, 4);
		...
		partial void OnBackBtn_Pressed (SDXButton sender)
		{
			...
			var prevView = decisionViews.Last ();
				...
				ActionsView.Hidden = true;
				UIView.Animate (0.3f, () => {
					Util.MoveViewToX (currentView, 620);
				...
				imgManager.LoadCompleted += (object s, byte[] bytes) => {
					InvokeOnMainThread (delegate {
						...
						if (bytes == null) {
							...
							Util.MoveViewToX (PlanogramView, 620);
							UIView.Animate (0.3f, () => {
								Util.MoveViewToX (currentView, -620);
		...
		async partial void OnSetAsPlanogramBtn_Pressed (SDXButton sender)
		{
			...
			PostChangeView(DetailType.OFFER_DETAIL);
		}
		#endregion
